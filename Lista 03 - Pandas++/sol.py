...
def count_missing(df) -> int:
    data_null = pd.isnull(df).sum()
    return data_null.sum()
...
def delay(df) -> tuple:
    return (drop_missing(df)['DepDelay'].mean(), drop_missing(df)['ArrDelay'].mean())
...
cancelado= df['Cancelled'] == 1
data = df.drop(df[cancelado].index)

def high_delay(df):
    cancelado = df['Cancelled'] == 1
    data = df.drop(df[cancelado].index)
    data['rota'] = np.zeros(data.shape[0])
    dia = data.CRSArrTime >= data.CRSDepTime
    data.loc[dia, 'rota'] = data.CRSArrTime - data.CRSDepTime
    data.loc[~dia, 'rota'] = 2400 - data.CRSDepTime + data.CRSArrTime
    voo = data.groupby(['OriginAirportName','DestAirportName'])['rota'].mean()
    f = voo[voo == voo.max()].to_frame().index.tolist()
    return (f[0][0], f[0][1])
...
x = df.drop(df[cancelado].index)
plt.scatter(x.DepDelay, x.ArrDelay, edgecolor='k')
plt.xlabel('DepDelay')
plt.ylabel('ArrDelay')