# -*- coding: utf-8 -*-
"""lista4_bootstrap.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1We7PA1RzLlmfhBuKEAdg5jWmVRWyj0l9
...
def CI(array):
  ...
  # return (LI, LS)
  a = 0.99
  filtro = array[array.DEPENDENCIA_ADMINISTRATIVA == 'Estadual']
  N = len(filtro)
  LI = filtro.mean() - ss.norm.ppf(a+(1-a)/2).round(2) * (filtro.std(ddof=1) / np.sqrt(N))
  LS = filtro.mean() + ss.norm.ppf(a+(1-a)/2).round(2) * (filtro.std(ddof=1) / np.sqrt(N))
...
filtro = data[data.DEPENDENCIA_ADMINISTRATIVA == 'Federal']
...
def bootstrap_mean(n_sub, alpha):
  ...
  for i in range(n_sub):
    sample = filtro.sample(size, replace=True, random_state=i)
    values[i] = sample[col].mean()
...
def bootstrap(n_sub, col, dep_adm):
  ...
  values = np.zeros(n_sub)
  filtro = data[data.DEPENDENCIA_ADMINISTRATIVA == dep_adm]
  size = len(filtro)
