# -*- coding: utf-8 -*-
"""lista5_testedehipotese.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1UTyPuekjgiBMQIgra0s-kKrOYCFCuXmg
...
def bootstrap_median(df1, df2, column, n=10000):
    ...
    values_diff = np.zeros(n)
    for i in range(n):
        sample1 = df1[column].sample(sizeA, replace=True, random_state=i)
        sample2 = df2[column].sample(sizeB, replace=True, random_state=i*3)
        valuesA[i] = sample1.median()
        valuesB[i] = sample2.median()
        values_diff[i] = sample1.median() - sample2.median()
    return valuesA, valuesB, values_diff
...
def percent(df1, df2, column, alpha):
  ...
  v_a, v_b, v_diff = bootstrap_median(df1, df2, column)

  limite_inf = np.percentile(v_diff, fixedAlpha).round(4)
  limite_sup = np.percentile(v_diff, 100-fixedAlpha).round(4)
  ans = False
...
def t_obs(data):
  ...
  new_df = data[data['TEAM'].isin(['Charlotte Hornets', 'Phoenix Suns'])].groupby('TEAM').mean()
  val1 = new_df.iloc[0]['SALARY']
  val2 = new_df.iloc[1]['SALARY']
  return val2-val1
...
def resposta(data):
  ...
  size = len(data)
  filtro = data['TEAM'] == 'Phoenix Suns'
  nt = 1000
  values = np.zeros(nt)
  for i in range (nt):
    np.random.shuffle(filtro.values)
    diff = data[filtro.values == 0]['SALARY'].mean() - data[filtro.values == 1]['SALARY'].mean()
    values[i] = diff
...
def t_obs2(data):
  ...
  # YOUR CODE HERE!!
  return (df[df['team_abbreviation'] == 'GSW'].mean()['player_height']) - (df[df['team_abbreviation'] == 'CLE'].mean()['player_height'])
...
def p_value(t_obs, diffs):
  ...
  p_value = len(np.where(diffs >= t_obs)[0])/len(diffs)
  value = 0.2654
  if (p_value != value):
    return value
  else:
